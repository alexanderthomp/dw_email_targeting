...
server <- function(input, output) { 
  ...
  output$dateui <- renderUI({
    switch(input$date,
           "sales between dates" = radioButtons(inputId = "daterange", label = "sales window", choices = c("last two weeks", "this time last year"))
    )
  })
  ...
  tab2 <- eventReactive(input$go, { 
    isolate(withProgress(message="Searching ...",{
      ...
      if(input$date == "recently published"){
        pubDate <- "published_date > CURRENT_DATE - 14"
        viewDate1 <- "CURRENT_DATE - 14"
        viewDate2 <- "CURRENT_DATE"
      }else
      {pubDate <- "published_date IS NOT NULL"
      if(input$daterange == "last two weeks"){
        viewDate1 <- "CURRENT_DATE - 14"
        viewDate2 <- "CURRENT_DATE"
      }else
      {
        viewDate1 <- "CURRENT_DATE - 365 - 14"
        viewDate2 <- "CURRENT_DATE"
      }
      }
